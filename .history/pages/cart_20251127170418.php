...
<script>
    document.getElementById('submitBuyerInfo').addEventListener('click', function() {
    ...
    // ⚡ Lưu dữ liệu hoặc gửi lên server
    console.log({name, phone, address, note});

    Swal.fire({
        title: 'Cảm ơn bạn!',
        text: 'Thông tin đã được ghi nhận. Shop sẽ liên hệ bạn sớm.',
        icon: 'success'
    });
...
function initMap() {
    ...
    const map = new google.maps.Map(mapDiv, {
        center: {lat: 10.762622, lng: 106.660172}, // ví dụ: trung tâm TP.HCM
        zoom: 16
    });
    ...
    // Khi marker thay đổi vị trí, có thể lưu tọa độ
    marker.addListener('dragend', function(event) {
        console.log("Vị trí mới:", event.latLng.lat(), event.latLng.lng());
    });
}
</script>
